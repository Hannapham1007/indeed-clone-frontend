...
import { updateJobPost } from "../services/api";
...
function EditJobPost() {
  ...
  const updatePost = async () => {
    const response = await updateJobPost(formData, token, id);
    let updatedPost;
    if (response && response.data) {
      setPosts((prevPosts) =>
         updatedPost = prevPosts.map((post) => (post.id === id ? response.data : post))
      );

      navigate("/my_job_posts");
      ...
      <div className="container large-space">
        ...
        <div className="d-flex align-items-center rounded-4 px-4 justify-content-between bg-white mt-4 py-4">
          <form onSubmit={handleSubmit} className="w-100">
            ...
            <button type="submit" className="btn btn-outline mx-1" onClick={()=> navigate('/my_job_posts')}>
                Cancel
              </button>
